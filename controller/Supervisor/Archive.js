...
const ArchiveManagmentController = {
    async getAllArchivedCount(req, res, next) {
        try {
            ...
            res.status(httpStatus.OK).send({ topups: 'Upcoming', promocodes: 'Upcoming', others: 'Upcoming', subjects, levels, topics, subtopics, vocabs, pastpapers, papernos, sheets, books, tasks });
        } catch (err) {
            next(err);
        }
    },
    ...
    async unArchiveSubject(req, res, next) {
        try {
            ...
            let levels = await SubjectLevel.update({ isArchived: false }, {
                where: {
                    id: req.body.levels
                }
            });
    ...
    async unArchiveSubjectLevel(req, res, next) {
        try {
            let subjectlevel = await SubjectLevel.update({ isArchived: false }, { where: { id: req.body.id } })
    ...
    async getAllArchivedTopics(req, res, next) {
        try {
                ...
                include: [
                    {
                        model: TopicTask,
                        attributes: ["id", "grade"],
                        include: [{ model: Board, attributes: ["id", "boardName"] }, { model: Subject, attributes: ["id"], include: [{ model: subjectName, attributes: ["subjectName"] }] }, { model: SubBoard, attributes: ["id", "subBoardName"] }]
                    },
                    {
                        model: Topic,
                        attributes: ["id", "name"],
            ...
            for (let i = 0; i < topics.length; i++) {
                var subtopics = await TaskSubTopicMapping.findAll({ where: { topicTaskId: topics[i].topicTaskId, topicId: topics[i].topicId }, include: [{ model: SubTopic }], attributes: ["isArchived"] })

                var vocabs = await TaskVocabularyMapping.findAll({ where: { topicTaskId: topics[i].topicTaskId, topicId: topics[i].topicId }, include: [{ model: Vocabulary }], attributes: ["isArchived"] })
    ...
    async unArchiveTopic(req, res, next) {
        try {
            var topics = await TaskTopicMapping.update({ isArchived: false }, { where: { id: req.body.id } })
            var subtopics = await TaskSubTopicMapping.update({ isArchived: false }, { where: { subTopicId: req.body.subTopics, topicId: req.body.id } })
            var vocabs = await TaskVocabularyMapping.update({ isArchived: false }, { where: { vocabularyId: req.body.vocabs, topicId: req.body.id } })

            res.status(httpStatus.OK).send({ message: "UnArchived Succesfully", topics, subtopics, vocabs });
        } catch (err) {
            console.log(err)
            next(err);
        }
    ...
    async getAllArchivedSubTopics(req, res, next) {
        try {
            ...
            for (let i = 0; i < subtopics.length; i++) {
                ...
                var isTopicArchived = await TaskTopicMapping.findAll({ where: { topicId: subtopics[i].topicId, topicTaskId: subtopics[i].topicTaskId }, attributes: ["isArchived"] })
                response.push({ 'topic': topicData[0], 'subtopic': subtopics[i], 'isTopicArchived': isTopicArchived[0].isArchived })
            }
    ...
    async getAllArchivedVocabs(req, res, next) {
        try {
            ...
            for (let i = 0; i < vocabs.length; i++) {
                ...
                var isTopicArchived = await TaskTopicMapping.findAll({ where: { topicId: vocabs[i].topicId, topicTaskId: vocabs[i].topicTaskId }, attributes: ["isArchived"] })
                response.push({ 'topic': topicData[0], 'vocab': vocabs[i], 'isTopicArchived': isTopicArchived[0].isArchived })
            }
                ...
                include: [
                    {
                        ...
                        include: [
                            ...
                            { model: Subject, attributes: ["id"], include: [{ model: subjectName, attributes: ["subjectName"] }] },
    ...
    async getAllBooks(req, res, next) {
        try {
            var books = await Book.findAll({ where: { isArchived: true }, attributes: ["id", "name", "subTitle", "author", "publisher"] })
            ...
            for (var i = 0; i < books.length; i++) {
                ...
                var chapCount = await TaskBookChapterMapping.count({ where: { bookTaskId: JSON.parse(JSON.stringify(bookTask[0])).bookTaskId, bookId: books[i].id } })
                response.push({ 'chapters': chapCount, 'book': books[i], ...JSON.parse(JSON.stringify(bookTask[0])) })
            }
        ...
        try {
            ...
            for (var i = 0; i < allSubjectIds.length; i++) {
                if (map.has(allSubjectIds[i].subjectNameId)) {
                    ...
                    map.set(allSubjectIds[i].subjectNameId, [allSubjectIds[i].id])
            }

            var subjectIds = [];
            map.forEach((value, key) => { subjectIds.push({ subjectNameId: key, id: value }) })
    ...
    async getArchivedSheetsBySubjectId(req, res, next) {
        try {
            ...
            for (var i = 0; i < sheets.length; i++) {
                var user = await User.findOne({ where: { id: sheets[i].assignedToUserId }, attributes: ["id", "Name"] })
                var subjectLevel = await SubjectLevel.findAll({ where: { id: sheets[i].subjectLevelId }, attributes: ["id", "subjectLevelName"] })
                sheets[i] = { ...JSON.parse(JSON.stringify(sheets[i])), 'assignedUser': user, 'subjectLevel': subjectLevel[0] }
            }
    ...
    async unArchiveSheet(req, res, next) {
        try {
            var sheetManagement = await SheetManagement.update({ isArchived: false }, { where: { id: req.body.id } })
    ...
    async unArchiveTask(req, res, next) {
        try {
            ...
            if (details.TaskType === 'book') {
                ...
                throw new ApiError(httpStatus.BAD_REQUEST, "Task Not Found For This Type!");
